...
type ScaleCalculator struct {
	...
	instanceType     string // instance type for capacity lookup
...
// The instanceType is used to look up node capacity via the capacityProvider.
// If capacityProvider is nil, only existing node capacity will be used.
func NewScaleCalculator(pool *stratosv1alpha1.NodePool, instanceType string, capacityProvider cloudprovider.InstanceCapacityProvider) *ScaleCalculator {
...
// 2. Fall back to static mapping via the capacity provider
func (c *ScaleCalculator) getNodeCapacity(existingNodes []corev1.Node) cloudprovider.InstanceCapacity {
	...
	for _, node := range existingNodes {
		if node.Status.Allocatable != nil {
			...
			if !cpu.IsZero() && !mem.IsZero() {
				return cloudprovider.InstanceCapacity{CPU: cpu, Memory: mem}
			}
		}
	}

	// Priority 2: Static mapping via capacity provider
	if c.instanceType != "" && c.capacityProvider != nil {
		return c.capacityProvider(c.instanceType)
	}

	return cloudprovider.InstanceCapacity{} // Unknown - will fall back to 1:1
}
