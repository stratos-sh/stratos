...
func TestPodAssignmentTTL(t *testing.T) {
	if nodestate.PodAssignmentTTL != 10*time.Minute {
		t.Errorf("PodAssignmentTTL = %v, want 10m", nodestate.PodAssignmentTTL)
	}
}
...
func TestEstimatePodsPerNode_FallsBackToTotal(t *testing.T) {
	...
	calc := scaling.NewScaleCalculator(nodePool, "unknown.type", aws.GetInstanceCapacity)
...
func TestEstimatePodsPerNode_ResourceBased(t *testing.T) {
	...
	calc := scaling.NewScaleCalculator(nodePool, "m5.xlarge", aws.GetInstanceCapacity)
	nodesNeeded := calc.CalculateNodesNeeded(pods, nil)
	if nodesNeeded != 2 {
...
func TestFilterAssignedPods_NoAssignments(t *testing.T) {
	...
	// filterAssignedPods is a method on Reconciler, but the "no assignments"
...
// makePodsWithoutRequests creates n pods with no resource requests.
